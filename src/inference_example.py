# -*- coding: utf-8 -*-
"""Inference example — full GPT-2 + H13 adapter, CPU-friendly."""
...
ADAPTER_NPZ = BASE_DIR / "adapter_torch_EN_BFCL.npz"  # classifier npz
...
# Load adapter classifier weights
...
m.freeze_gpt()  # freeze base GPT-2
m.to(DEVICE)
m.eval()
...
with torch.no_grad():
    for _ in range(40):
        ...
        # Берём первый элемент из кортежа (логиты)
        logits, *_ = m(input_ids, mask)
...
result = decode(ids[len(encode(prompt)):])
print("\n[RESULT]")
print(result)